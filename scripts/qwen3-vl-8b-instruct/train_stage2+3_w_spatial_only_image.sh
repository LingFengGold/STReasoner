...
MODEL_PATH=/workspace/output/Qwen3-VL-8B-Instruct-stage2-only-image  # replace it with your local file path

python3 -m src.EasyR1.verl.trainer.main \
    config=./src/EasyR1/examples/config.yaml \
    data.train_files=./data/ST-Bench/ST-RL-image/etiological_rl.jsonl,./data/ST-Bench/ST-RL-image/forecasting_rl.jsonl,./data/ST-Bench/ST-RL-image/correlation_rl.jsonl,./data/ST-Bench/ST-RL-image/entity_rl.jsonl \
    ...
    worker.rollout.n=8 \
    trainer.experiment_name=qwen3_8b_grpo_stage2+3_w_spatial_only_image \
