...
class Qwen2TSForCausalLM(Qwen2PreTrainedModel):
    ...
    self, time_series_features, inputs_embeds, input_ids, attention_mask, labels, patch_cnt
        ...
        # ===== VALIDATE AND FIX MISMATCHED TS TOKENS =====
        num_start_tokens = torch.sum(special_ts_token_mask_start, dim=-1)
        num_end_tokens = torch.sum(special_ts_token_mask_end, dim=-1)
        
        # Check for mismatched pairs
        if not torch.all(num_start_tokens == num_end_tokens):
            print(f"\n⚠️  WARNING: Mismatched <TS> and </TS> tokens detected!")
            # Clone tensors to avoid inplace operations on gradient-tracked tensors
            ...
            for i in range(batch_size):
                if num_start_tokens[i] != num_end_tokens[i]:
                    start_cnt = num_start_tokens[i].item()
                    end_cnt = num_end_tokens[i].item()
                    print(f"  Sample {i}: {start_cnt} <TS> but {end_cnt} </TS> (diff: {end_cnt - start_cnt})")
                    
                    # Auto-fix: remove extra end tokens or add missing start tokens
                    if end_cnt > start_cnt:
                        # Too many end tokens - remove the extras
                        end_positions = (input_ids[i] == self.config.ts_token_end_index).nonzero(as_tuple=True)[0]
                        # Remove the last extra end tokens
                        extra_count = end_cnt - start_cnt
                        tokens_to_remove = end_positions[-extra_count:]
                        print(f"    → Removing {extra_count} extra </TS> token(s) at positions {tokens_to_remove.tolist()}")
                        # Replace with pad token or a neutral token
                        for pos in tokens_to_remove:
                            input_ids[i, pos] = self.config.pad_token_id
                            attention_mask[i, pos] = 0
                            if labels is not None:
                                labels[i, pos] = self.config.ignore_index
                    elif start_cnt > end_cnt:
                        # Too many start tokens - remove the extras
                        start_positions = (input_ids[i] == self.config.ts_token_start_index).nonzero(as_tuple=True)[0]
                        extra_count = start_cnt - end_cnt
                        tokens_to_remove = start_positions[-extra_count:]
                        print(f"    → Removing {extra_count} extra <TS> token(s) at positions {tokens_to_remove.tolist()}")
                        for pos in tokens_to_remove:
                            input_ids[i, pos] = self.config.pad_token_id
                            attention_mask[i, pos] = 0
            ...
            # Recalculate masks after fixing
            special_ts_token_mask_start = input_ids == self.config.ts_token_start_index
            special_ts_token_mask_end = input_ids == self.config.ts_token_end_index
            special_ts_token_mask = special_ts_token_mask_start | special_ts_token_mask_end
            print(f"  ✓ Fixed mismatched tokens\n")
        # ===== END VALIDATION =====

        # print("num TS tokens in input:", special_ts_token_mask_start.sum(dim=1)) # tensor([5, 5, 5, 5, 5, 5, 5, 5], device='cuda:1') 
        # print("len(patch_cnt):", len(patch_cnt)) # 40
        ...
        # ===== DEBUGGING: Check for data mismatch =====
        total_expected_ts = num_special_ts_tokens.sum().item()
        if len(patch_cnt) != total_expected_ts:
            print(f"\n⚠️  WARNING: patch_cnt length mismatch!")
            print(f"  Expected (from input_ids): {total_expected_ts}")
            print(f"  Actual (patch_cnt): {len(patch_cnt)}")
            print(f"  Difference: {total_expected_ts - len(patch_cnt)}")
            
            # Try to fix by truncating or padding patch_cnt
            if len(patch_cnt) > total_expected_ts:
                # Too many patches - truncate
                print(f"  → Truncating patch_cnt from {len(patch_cnt)} to {total_expected_ts}")
                patch_cnt = patch_cnt[:total_expected_ts]
                # Also need to truncate time_series_features
                total_features = patch_cnt.sum().item()
                time_series_features = time_series_features[:total_features]
                print(f"  → Truncated time_series_features to {time_series_features.shape}")
            else:
                # Not enough patches - this is more problematic
                error_msg = (
                    f"\n{'='*80}\n"
                    f"❌ CRITICAL DATA MISMATCH - INSUFFICIENT DATA!\n"
                    f"{'='*80}\n"
                    f"Expected TS tokens (from input_ids): {total_expected_ts}\n"
                    f"Actual patch_cnt length: {len(patch_cnt)}\n"
                    f"Missing: {total_expected_ts - len(patch_cnt)} time series data\n"
                    f"\nBatch Details:\n"
                    f"  - batch_size: {batch_size}\n"
                    f"  - sequence_length: {sequence_length}\n"
                    f"  - time_series_features.shape: {time_series_features.shape}\n"
                    f"  - patch_cnt: {patch_cnt.tolist()}\n"
                    f"\nPer-sample TS token counts:\n"
                )
                for i in range(batch_size):
                    ts_count = num_special_ts_tokens[i].item()
                    ts_positions = (input_ids[i] == self.config.ts_token_start_index).nonzero(as_tuple=True)[0]
                    error_msg += f"  Sample {i}: {ts_count} TS tokens at positions {ts_positions.tolist()}\n"
                
                error_msg += (
                    f"\nCannot proceed - not enough time series data provided!\n"
                    f"This indicates a data pipeline bug where time series data was lost.\n"
                    f"{'='*80}\n"
                )
        ...
        for i in range(batch_size):
            ...
            # num_total_patches[i] = patch_cnt[patch_index : patch_index + num_ts_in_batch].sum()
            for idx in range(patch_index, patch_index + num_ts_in_batch):
                # Check bounds before accessing
                if idx >= len(special_ts_token_mask_start_nonzero):
                    error_msg = (
                        f"\n{'='*80}\n"
                        f"❌ INDEX OUT OF BOUNDS ERROR!\n"
                        f"{'='*80}\n"
                        f"Trying to access index {idx} but special_ts_token_mask_start_nonzero has length {len(special_ts_token_mask_start_nonzero)}\n"
                        f"\nLoop State:\n"
                        f"  - batch_sample: {i}/{batch_size}\n"
                        f"  - patch_index: {patch_index}\n"
                        f"  - num_ts_in_batch: {num_ts_in_batch}\n"
                        f"  - current idx: {idx}\n"
                        f"  - range: [{patch_index}, {patch_index + num_ts_in_batch})\n"
                        f"\nData State:\n"
                        f"  - len(patch_cnt): {len(patch_cnt)}\n"
                        f"  - num_special_ts_tokens: {num_special_ts_tokens.tolist()}\n"
                        f"  - special_ts_token_mask_start total: {torch.sum(special_ts_token_mask_start)}\n"
                        f"{'='*80}\n"
                    )
                ...
                # special_ts_token_mask_start_with_size[b_idx, pos] *= patch_cnt[idx].item()
        ...
        if ts_to_overwrite.sum() != time_series_features.shape[:-1].numel():
            error_msg = (
                f"\n{'='*80}\n"
                f"❌ TIME SERIES FEATURE COUNT MISMATCH!\n"
                f"{'='*80}\n"
                f"Positions to fill (ts_to_overwrite): {ts_to_overwrite.sum()}\n"
                f"Available features: {time_series_features.shape[:-1].numel()}\n"
                f"Difference: {ts_to_overwrite.sum() - time_series_features.shape[:-1].numel()}\n"
                f"\nDetailed Breakdown:\n"
                f"  - special_ts_token_mask_start count: {torch.sum(special_ts_token_mask_start)}\n"
                f"  - special_ts_token_mask_end count: {torch.sum(special_ts_token_mask_end)}\n"
                f"  - len(patch_cnt): {len(patch_cnt)}\n"
                f"  - sum(patch_cnt): {patch_cnt.sum().item()}\n"
                f"  - patch_cnt values: {patch_cnt.tolist()}\n"
                f"\nShape Information:\n"
                f"  - time_series_features: {time_series_features.shape}\n"
            ...
            for i in range(batch_size):
                ts_count = num_special_ts_tokens[i].item()
                ts_end_count = torch.sum(input_ids[i] == self.config.ts_token_end_index).item()
                error_msg += (
                    f"  Sample {i}:\n"
                    f"    - TS start tokens: {ts_count}\n"
                    f"    - TS end tokens: {ts_end_count}\n"
                    f"    - num_total_patches: {num_total_patches[i].item()}\n"
                )
            
            error_msg += (
                f"\nPossible Causes:\n"
                f"  1. Mismatch between <TS> token pairs in input_ids\n"
                f"  2. patch_cnt calculation error in ts_encoder\n"
                f"  3. Data preprocessing issue (truncation/padding)\n"
                f"  4. Corrupted batch data\n"
                f"{'='*80}\n"
            )
            raise ValueError(error_msg)
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if timeseries is not None and timeseries.shape[0] > 0:
                ...
                # print("input_ids batch:", input_ids.size(0)) # 8
                # print("timeseries batch:", timeseries.size(0)) # 40
